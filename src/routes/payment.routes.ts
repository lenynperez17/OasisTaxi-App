...
router.post('/process-yape', [
  ...
  try {
    ...
    const paymentData = {
      ...
      createdAt: Timestamp.now(),
      metadata: {
        qrCode: `yape://payment?amount=${amount}&phone=946123456`, // Número de Oasis Taxi
    ...
    const yapeQRData = {
      phoneNumber: '946123456', // Número de Oasis Taxi para recibir pagos
      amount,
...
router.post('/process-plin', [
  ...
  try {
    ...
    const plinQRData = {
      phoneNumber: '946123456', // Número de Oasis Taxi
      amount,
      message: `Viaje #${rideId}`,
